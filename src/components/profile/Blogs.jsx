...
const Blogs = ({ blogs, userProfile, user }) => {
    ...
    <Flex direction={"column"} gap={"10px"}>
      {blogs?.map((each) => (
        <Box key={each._id} className="bg-green-light-9" p={"20px"}>
          <Link to={`/${each?.slug}`}>
            <Text className="fw-bold">{each.title}</Text>
          </Link>
          <Text className="small-text">
            Tags:{" "}
            {each.tags.map((eac, index) => (
              <Text as={"span"} key={index} className="small-text">
                {eac}
                {index === each.tags.length - 1 ? "." : ","}{" "}
              </Text>
            ))}
          </Text>
          <Text className="tiny-text" mt={"10px"}>
